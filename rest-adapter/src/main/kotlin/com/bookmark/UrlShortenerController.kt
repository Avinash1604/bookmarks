...
@Tag(name = "shortUrl", description = "This allows user to dealing with big urls problems by shorting it")
...
class UrlShortenerController(private val bookmarkService: BookmarkService) {
    ...
    @ApiResponses(value = [
        ApiResponse(responseCode = "200", description = "Successfully created short url", content = [
            (Content(mediaType = "application/json", array = (
                    ArraySchema(schema = Schema(implementation = Url::class)))))]),
        ApiResponse(responseCode = "400", description = "Bad request", content = [Content(array = (
                ArraySchema(schema = Schema(implementation = ExceptionResponse::class))))]),
    ...
    fun createShortUrl(@RequestBody urlRequest: UrlRequest): ResponseEntity<Url> {
        return ResponseEntity.status(HttpStatus.CREATED).body(bookmarkService.createShortUrl(urlRequest))
    }
}